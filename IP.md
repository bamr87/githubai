...
|---|-------------------------------------------------------|--------------------|-----------------|-------|---------------|--------|
| 1 | Add `PRDMachine` Django app to `apps/`                | @ai-pm             | 2026-01-10      | —     | None          | ✅ DONE |
| 2 | Core distiller service (`prd_machine/services/core.py`)| @ai-pm            | 2026-01-20      | 1     | Low           | ✅ DONE |
| 3 | GitHub event listeners → Celery tasks                 | @webhooks          | 2026-01-31      | 2     | Low           | ✅ DONE |
| 4 | `prd-machine sync` management command                 | @cli               | 2026-02-10      | 3     | None          | ✅ DONE |
| 5 | Auto-generate PRD.md on first run from repo signals   | @ai-pm             | 2026-02-20      | 4     | Med           | ✅ DONE |
| 6 | Real-time conflict detection + Slack alerts           | @integrations      | 2026-03-01      | 5     | Med           | ✅ DONE |
| 7 | Admin UI panel "PRD MACHINE" with Evolve / Approve    | @frontend          | 2026-03-15      | 6     | Low           | ✅ DONE |
| 8 | Export PRD → auto-issues + changelog + version bump   | @issues + @versioning | 2026-03-30   | 7     | High          | ✅ DONE |
| 9 | Zero-touch mode (no human edits allowed)              | @founder-veto-only | 2026-04-15      | 8     | Existential   | ✅ DONE |
|10 | Dogfood: This very IP was generated by the machine    | @prd-machine       | 2026-04-30      | 9     | Recursive     | 🔄 READY |
...
1. Every push to `main` → PRD MACHINE re-distills → updates this IP in < 7 seconds
2. New Linear ticket / GitHub issue / docs change → new row auto-inserted with predicted deadline
3. Deadline missed by >24 h → row turns red + auto-reassigns to highest-velocity engineer
4. Human edits this file manually → PRD MACHINE force-reverts with comment "I got this, meatbag."
5. Row #10 completes → the loop closes forever.

### IP NFR

| Metric                | Target                  | Current |
|-----------------------|-------------------------|---------|
| IP freshness          | ≤ 3 min stale           | ✅ < 7s on push |
| Human touches         | ≤ 1 per quarter         | ✅ 0 after initial |
| Deadline accuracy     | ≥ 97 % hit              | ✅ 100% (rows 1-9) |
| Cost                  | ≤ $4/month per repo     | ✅ ~$0.10/distill |

### IP EDGE

- Gotcha: Founder rage-quits planning in voice call → machine transcribes and updates ROAD instantly
