...
def test_load_template(mocker):
    mock_load_template_from_path = mocker.patch(
        "src.githubai.create_issue.load_template_from_path"
    )
    mock_load_template_from_path.return_value = (
        {"prompt": "Test prompt"},
        "Template body",
    )
...
def test_create_readme_update_issue(mocker):
    ...
    mock_fetch_file_contents = mocker.patch(
        "src.githubai.create_issue.fetch_file_contents"
    )
    mock_call_openai_with_prompt = mocker.patch(
        "src.githubai.create_issue.call_openai_with_prompt"
    )
    mock_create_github_issue = mocker.patch(
        "src.githubai.create_issue.create_github_issue"
    )

    mock_fetch_issue.return_value = {"body": "Issue body", "title": "Issue title"}
    mock_load_template_from_path.return_value = (
        {"prompt": "Test prompt", "include_files": []},
        "Template body",
    )
...
def test_create_sub_issue_from_template_with_file_extraction(mocker):
    ...
    mock_fetch_issue.return_value = {
        "body": "Issue body with include_files_additional: - file1\n- file2",
        "title": "Issue title",
    }
    mock_load_template.return_value = (
        {"prompt": "Test prompt", "include_files": ["file3"]},
        "Template body",
        "Test prompt",
        "[Structured]: ",
    )
...
def test_run_create_issue(mocker):
    mock_create_sub_issue_from_template = mocker.patch(
        "src.githubai.create_issue.create_sub_issue_from_template"
    )
    mock_create_sub_issue_from_template.return_value = {
...
def test_parse_args_for_sub_issue(mocker):
    mocker.patch(
        "sys.argv", ["create_issue", "--repo", "repo", "--parent-issue-number", "1"]
    )
...
def test_main(mocker):
    mock_parse_args_for_sub_issue = mocker.patch(
        "src.githubai.create_issue.parse_args_for_sub_issue"
    )
    mock_run_create_issue = mocker.patch("src.githubai.create_issue.run_create_issue")

    mock_parse_args_for_sub_issue.return_value = mocker.Mock(
        repo="repo", parent_issue_number="1", file_refs=[]
    )
...
def test_logging_of_prompt_and_files(mocker):
    ...
    mock_call_openai_chat = mocker.patch(
        "src.githubai.utils.openai_utils.call_openai_chat"
    )
    mock_call_openai_chat.return_value = "AI response"
