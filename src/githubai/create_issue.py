...
def fetch_file_contents(repo, filepath):
    """
    Fetch file contents from a specified file in a GitHub repository.

    This function makes a GET request to the GitHub API to retrieve a file's content, decodes it from base64,
    and returns the decoded content as a UTF-8 string.

    Args:
        repo (str): Repository identifier in the format "owner/repo".
    ...
    Raises:
        ...
        KeyError: If the retrieved JSON does not contain the expected 'content' field.
    """
    """Fetch file contents from GitHub repository."""
    res = requests.get(
        f"https://api.github.com/repos/{repo}/contents/{filepath}", headers=HEADERS
    )
...
def call_openai_with_prompt(promptAndTemplate, issueContent):
    """Call OpenAI with a formatted prompt."""
    messages = [
        {"role": "system", "content": promptAndTemplate},
...
def create_readme_update_issue(repo, issue_number):
    ...
    yaml_config, template_body = load_template_from_path(
        ".github/ISSUE_TEMPLATE/README_update.md"
    )

    # Collect all files to include
    include_files = yaml_config.get("include_files", [])
    # Look for additional files in the issue body
    include_files_additional_match = re.search(
        r"include_files_additional:(.*?)##", issue["body"], re.DOTALL
    )
    ...
    systemInstructions = f"{yaml_config['prompt']}\n\n{template_body}"
    userInstructions = f"{issue['body']}\n\nIncluded Files:{included_files_content}"
    ai_content = call_openai_with_prompt(systemInstructions, userInstructions)
...
def create_sub_issue_from_template(repo, parent_issue_number, file_refs=None):
    ...
    yaml_config, template_body, prompt, issue_title_prefix = load_template(
        template_name
    )
    ...
    issue_file_paths = re.findall(
        r"include_files_additional:\s*-\s*(.*?)\s*$", parent_body, re.MULTILINE
    )
    ...
    systemInstructions = f"{prompt}\n\n{template_body}"
    userInstructions = parent_body
    ai_generated_body = call_openai_with_prompt(systemInstructions, userInstructions)
...
def parse_args_for_sub_issue():
    ...
    parser = argparse.ArgumentParser(
        description="Create GitHub sub-issues with AI-generated content"
    )
    parser.add_argument(
        "--repo", required=True, help="GitHub repository in format 'owner/repo'"
    )
    parser.add_argument(
        "--parent-issue-number", required=True, help="Parent issue number"
    )
    parser.add_argument(
        "--file-refs", nargs="*", default=[], help="Optional file references"
    )
...
def main():
    ...
    if "create_issue" in script_name:
        ...
        parser = argparse.ArgumentParser(
            description="Create GitHub issues with AI-generated content"
        )
        parser.add_argument(
            "--repo", required=True, help="GitHub repository in format 'owner/repo'"
        )
        ...
        parser.add_argument(
            "--parent-issue-number", help="Parent issue number for creating sub-issues"
        )
        parser.add_argument(
            "--file-refs", nargs="*", default=[], help="Optional file references"
        )
        ...
        if not args.issue_number and not args.parent_issue_number:
            parser.error(
                "Either --issue-number or --parent-issue-number must be specified"
            )
